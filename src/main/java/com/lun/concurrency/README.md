...
**[LiftOff](basic/LiftOff.java)** 实现Runnable接口，创建线程

**[BasicThreads](basic/BasicThreads.java)** 实现Thread类，创建线程

**[MoreBasicThreads](basic/MoreBasicThreads.java)** 实现Thread类，创建多线程

## Executor 创建线程 ##

Executors **/ɪɡˈzɛkjətɚ/** are the prederred method for starting tasks in Java SE5/6

**[CachedThreadPool](basic/CachedThreadPool.java)** 创建线程池

**[FixedThreadPool](basic/FixedThreadPool.java)** 固定数量的线程池

A **[CachedThreadPool](basic/CachedThreadPool.java)** will generally create as many threads as it needs during the execution of a program and then will stop creating new threads as it recycles the old ones, so it's reasonable first choice as an Executor. Only if this approach causes problems do you need to switch to a **[FixedThreadPool](basic/FixedThreadPool.java)**.

**[SingleThreadExecutor](basic/SingleThreadExecutor.java)** is like a FixedThreadPool with a size of one thread.只有一个线程的线程池

If more than one task is submitted to a **[SingleThreadExecutor](basic/SingleThreadExecutor.java)**, the tasks will be queued and each task will run to completion结束 before the next task is begun, all using the same thread。

## Producing return values from tasks --- Callable ##

**[CallableDemo](basic/CallableDemo.java)** 有返回值的任务，需要实现Callable而非Runable
...
**[SleepingTask](basic/SleepingTask.java)** sleep()阻塞线程一会儿

## Priority ##

**[SimplePriorities](basic/SimplePriorities.java)** 设置线程优先级
...
**[SimpleDaemons](basic/SimpleDaemons.java)** 将线程设置为守护线程，主线程运行完成，守护线程也会随之终止

**[DaemonThreadFactory](basic/DaemonThreadFactory.java)** 实现ThreadFactory接口的守护线程工厂

**[DaemonFromFactory](basic/DaemonFromFactory.java)** Executor 通过**[DaemonThreadFactory](basic/DaemonThreadFactory.java)** 工厂借口创建守护线程

**[DaemonThreadPoolExecutor](basic/DaemonThreadPoolExecutor.java)** 更高一级的 守护线程创造的方法 通过继承ThreadPoolExecutor()

**[Daemons](basic/Daemons.java)** 守护线程里执行任务中所创建的线程也是守护线程（龙生龙，凤生凤）

**[DaemonsDontRunFinally](basic/DaemonsDontRunFinally.java)** Daemon 与 finally擦不出火花
...
**[SimpleThread](basic/SimpleThread.java)** 继承Thread类创建线程后，构造方法立即启动

**[SelfManaged](basic/SelfManaged.java)** 类似上例子，用Runnable接口

**[ThreadVariations](basic/ThreadVariations.java)** 内部类与Runable或Thread碰撞
...
**[Joining](basic/Joining.java)** join()用途,等其他线程跑完,自己接着跑
...
**[ResponsiveUI](basic/ResponsiveUI.java)** 一个创建守护线程的用户界面例子
...
**[ExceptionThread](basic/ExceptionThread.java)** 一个简单线程抛异常的例子

**[NaiveExceptionHandling](basic/NaiveExceptionHandling.java)** try-catch并不能捕获从线程抛出的异常

**[CaptureUncaughtException](basic/CaptureUncaughtException.java)** 通过实现Thread.UncaughtExceptionHandler接口方法来捕获线程抛出来的异常

**[SettingDefaultHandler](basic/SettingDefaultHandler.java)** 全局设置UncaughtExceptionHandler
