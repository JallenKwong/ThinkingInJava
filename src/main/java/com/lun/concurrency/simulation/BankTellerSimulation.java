...
class CustomerLine extends ArrayBlockingQueue<Customer2> {
	...
	public void run() {
		try {
			while (!Thread.interrupted()) {
				...
				customers.put(new Customer2(rand.nextInt(1000)));
			}
...
class Teller implements Runnable, Comparable<Teller> {
	...
	public void run() {
		try {
			while (!Thread.interrupted()) {
				Customer2 customer = customers.take();
